...
async function runScan(ctx) {
  ...
  for (const [module, list] of byModule) {
    ...
    for (const group of byConfig.values()) {
      ...
      conflicts.push({
        ...
        title: '插件重复挂载：' + module,
        detail: '模块 ' + module + ' 在同一组合中被以完全相同的配置加载了 ' + group.length + ' 次（'
          + group.map((r) => '条目“' + r.entryId + '”').join('、') + '）。完全相同的实例会重复注册服务、互相覆盖，导致行为不可预测。',
        where: views,
        action: stoppedNow > 0
          ? '已自动停用后加载的 ' + stoppedNow + ' 份实例，仅保留第一份（条目“' + kept.entryId + '”）'
          : '冲突实例此前已被停用，本次无需重复处理'
      });
    }
  }
  ...
  for (const rec of entries) {
    ...
    conflicts.push({
      ...
      action: result === 'stopped' ? '已自动停用该插件，避免其影响 DSH 稳定性' : '该插件已处于停止状态'
    });
  }
  ...
  for (const rec of entries) {
    ...
    conflicts.push({
      ...
      where: [pluginView(rec, result === 'stopped')],
      action: result === 'stopped' ? '已自动停用该插件，解除其挂起状态' : '该插件已处于停止状态'
    });
  }
  ...
  for (const rec of entries) {
    ...
    conflicts.push({
      ...
      action: restored ? '已自动恢复该插件实例，无需手动干预' : '该插件无法启动，已保持停用状态；请检查其配置或依赖'
    });
  }
  ...
  for (const [filename, rows] of collectFiles(loader)) {
    ...
    for (const [id, count] of idCount) {
      ...
      conflicts.push({
        ...
        severity: 'warning',
        title: '组合配置行 id 重复：' + id,
        detail: '配置文件 ' + filename + ' 中有 ' + count + ' 行使用了同一个 id “' + id
          + '”。重复的 id 会让加载器在下次启动时直接抛错（duplicate loader entry id），导致 DSH 无法启动。',
        where: [],
        action: '请手动修改配置文件，为其中一行指定唯一的 id（本插件不会修改你的配置文件）'
      });
    }
  }
  ...
  if (conflicts.length === 0) {
    ...
    summary = '检测到 ' + blocked + ' 个插件冲突（已自动处理）+ ' + warnings + ' 个提示';
  } else if (blocked > 0) {
    summary = '检测到 ' + blocked + ' 个插件冲突，冲突插件已被自动处理，DSH 保持稳定运行';
  } else {
    summary = '检测到 ' + warnings + ' 个提示，不影响本次运行';
  }
...
export function apply(ctx) {
  // Static install: webServer routes (the same convention dsh-omni-bridge and
  // dsh-session-mover use — client host.call(method) → POST /conflict-guardian/<method>).
  ctx.inject(['webServer'], (hostCtx) => {
    const server = hostCtx.webServer;
    if (!server || typeof server.register !== 'function') return;
    const guard = async (request, response, fresh) => {
      try {
        ...
        if (fresh) {
          lastReport = await runScan(ctx);
        } else if (!lastReport) {
          lastReport = await runScan(ctx);
        }
        sendJson(response, 200, lastReport);
      } catch (error) {
    ...
    server.register({
      kind: 'exact',
      path: '/conflict-guardian/report',
      handler: (request, response) => guard(request, response, false)
    });
    server.register({
      kind: 'exact',
      path: '/conflict-guardian/rescan',
      handler: (request, response) => guard(request, response, true)
    });
  });

  // Dynamic plugin runner (optional): harness.handle pairs with the client's
  // host.call, and the model-callable conflict_check tool becomes available.
  try {
    if (typeof harness !== 'undefined' && harness) {
      if (typeof harness.handle === 'function') {
        harness.handle('conflict-report', async () => {
          if (!lastReport) lastReport = await runScan(ctx);
          return lastReport;
        });
        harness.handle('conflict-rescan', async () => {
          lastReport = await runScan(ctx);
          return lastReport;
        });
      }
      if (typeof harness.defineTool === 'function' && typeof harness.registerTool === 'function') {
        harness.registerTool(ctx, harness.defineTool({
          name: 'conflict_check',
          description: '运行一次 DSH 插件冲突检测（重复挂载、启动失败、依赖缺失、配置行 id 重复），自动停用或恢复冲突插件，并返回检测报告。',
          parameters: {},
    ...
    // static environment has no harness — ignore
  }

  // Scan on startup (every DSH start), fire-and-forget.
  runScan(ctx).then((report) => {
    lastReport = report;
  }).catch((error) => {
    console.error('[dsh-conflict-guardian] startup scan failed', error);
    lastReport = {
      ok: false,
      scannedAt: Date.now(),
      summary: '启动扫描失败：' + messageOf(error),
      conflicts: []
    };
  });
}
